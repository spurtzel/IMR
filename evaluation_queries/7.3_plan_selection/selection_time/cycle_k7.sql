...
FROM events MATCH_RECOGNIZE (
    ...
    PATTERN (A Z* B Z* C Z* D Z* E Z* F Z* G)
    DEFINE
        ...
        G AS G.primary_type = 'T6' AND G.lon BETWEEN F.lon - 0.02 AND F.lon + 0.02 AND G.lat BETWEEN F.lat - 0.02 AND F.lat + 0.02 AND G.lon BETWEEN A.lon - 0.02 AND A.lon + 0.02 AND G.lat BETWEEN A.lat - 0.02 AND A.lat + 0.02,
        Z AS TRUE
)
