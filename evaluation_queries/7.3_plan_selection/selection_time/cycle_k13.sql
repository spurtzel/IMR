...
FROM events MATCH_RECOGNIZE (
    ...
    PATTERN (A Z* B Z* C Z* D Z* E Z* F Z* G Z* H Z* I Z* J Z* K Z* L Z* M)
    DEFINE
        ...
        M AS M.primary_type = 'T12' AND M.lon BETWEEN L.lon - 0.02 AND L.lon + 0.02 AND M.lat BETWEEN L.lat - 0.02 AND L.lat + 0.02 AND M.lon BETWEEN A.lon - 0.02 AND A.lon + 0.02 AND M.lat BETWEEN A.lat - 0.02 AND A.lat + 0.02,
        Z AS TRUE
)
