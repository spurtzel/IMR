...
FROM events MATCH_RECOGNIZE (
    ...
    PATTERN (A Z* B Z* C Z* D Z* E Z* F Z* G Z* H)
    DEFINE
        ...
        H AS H.primary_type = 'T7' AND H.lon BETWEEN G.lon - 0.02 AND G.lon + 0.02 AND H.lat BETWEEN G.lat - 0.02 AND G.lat + 0.02 AND H.lon BETWEEN A.lon - 0.02 AND A.lon + 0.02 AND H.lat BETWEEN A.lat - 0.02 AND A.lat + 0.02,
        Z AS TRUE
)
