...
FROM events MATCH_RECOGNIZE (
    ...
    DEFINE
        ...
        D AS D.primary_type = 'T3' AND D.lon BETWEEN C.lon - 0.02 AND C.lon + 0.02 AND D.lat BETWEEN C.lat - 0.02 AND C.lat + 0.02 AND D.lon BETWEEN A.lon - 0.02 AND A.lon + 0.02 AND D.lat BETWEEN A.lat - 0.02 AND A.lat + 0.02,
        Z AS TRUE
)
