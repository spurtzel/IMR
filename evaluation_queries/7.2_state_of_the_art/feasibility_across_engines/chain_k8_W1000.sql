...
FROM events MATCH_RECOGNIZE (
    ...
    DEFINE
        ...
        H AS H.primary_type = 'T7' AND H.lon BETWEEN G.lon - 0.02 AND G.lon + 0.02 AND H.lat BETWEEN G.lat - 0.02 AND G.lat + 0.02 AND H.ts BETWEEN A.ts AND A.ts + INTERVAL '1' SECOND,
        Z AS TRUE
)
