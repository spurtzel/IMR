...
FROM events MATCH_RECOGNIZE (
    ...
    DEFINE
        ...
        G AS G.primary_type = 'T6' AND G.lon BETWEEN F.lon - 0.02 AND F.lon + 0.02 AND G.lat BETWEEN F.lat - 0.02 AND F.lat + 0.02 AND G.ts BETWEEN A.ts AND A.ts + INTERVAL '1' SECOND,
        Z AS TRUE
)
